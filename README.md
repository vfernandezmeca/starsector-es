...
Descarga la [última versión](https://github.com/vfernandezmeca/starsector-es/releases/latest):
un zip que se arrastra a `mods/`.
...
**El parche** (opcional). Va dentro de la carpeta del mod, en `parche-menus/`,
así que se instala en el mismo arrastre. No hace falta instalar nada: usa la
Java que ya trae Starsector.
...
Que el parcheador viva ahí es comodidad, no que forme parte del mod: el juego
ignora las subcarpetas que no conoce y `parchear.jar` no está en el campo
`jars` del `mod_info.json`, así que ni lo mira.

El script guarda una copia `.orig` la primera vez y siempre parte de ella, así
que se puede repetir sin acumular daños. Cada actualización del juego borra el
parche: se vuelve a pasar.

Desde el repo, con Python, el equivalente es `tools/parchea.py` (ver
*Cómo está hecho*).

El `.jar` parcheado no se puede redistribuir: es el juego de Fractal Softworks
con cambios dentro. Lo que se reparte es el parcheador y el plan, y cada uno lo
aplica sobre su copia.

## Sobre los acentos

El mod va en ASCII puro: `Anos` en vez de `Años`. Starsector no carga las
fuentes de los mods, así que ningún carácter acentuado llega a dibujarse: sale
un apóstrofo en su lugar. Incluir fuentes parcheadas dentro del mod tampoco lo
arregla, los acentos siguen saliendo rotos igual.
...
Unas pocas palabras se quedan a propósito, porque el juego usa la misma cadena
como etiqueta y como identificador interno. Las más visibles son las pestañas
...
identificador es la parte delicada: seis fuentes de evidencia, siete roturas y
80 tests. Reimplementar eso en otro lenguaje sería reintroducir los mismos
...
El parcheador no decide nada, así que no puede equivocarse de entrada. Se
compila contra Java 8 para que corra con cualquier JRE que traiga el juego:
...
6.468 entradas byte a byte idénticas. Todo lo verificado con las herramientas
de Python sigue valiendo, porque el resultado es el mismo archivo.

Lo difícil no es traducir, es saber qué no traducir. Dentro de los jar conviven
texto de pantalla e identificadores con la misma pinta, y traducir un
identificador no da un error de traducción: el juego no arranca.

La lista de intocables (21.009 cadenas) no está escrita a mano. Sale de seis
evidencias del propio juego:

| Fuente | Qué detecta |
|---|---|
| Bytecode: el literal se compara o se pasa a un getter | claves de mapas, ids de pestaña |
| Constantes de enum (clase que extiende `java/lang/Enum`) | `MISSILE`, `GLOW`, que el juego busca con `valueOf` |
...
La decisión es global, no por clase: un identificador se registra en una clase
y se consulta en otra, y las dos tienen que seguir coincidiendo.

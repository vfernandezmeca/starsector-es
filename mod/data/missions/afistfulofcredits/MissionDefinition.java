...
//		api.getDefaultCommander(FleetSide.PLAYER).getStats().setSkillLevel(Skills.ELECTRONIC_WARFARE, 3);
		...
		api.setFleetTagline(FleetSide.PLAYER, "\"Solo soy un comerciante honrado que intenta ganarse la vida, oficial\".");
		api.setFleetTagline(FleetSide.ENEMY, "La traicionera y de alta radiacion Moon Salazar en un mulo oxidado");
		
		// These show up as items in the bulleted list under 
		// "Objetivos tacticos" on the mission detail screen
		api.addBriefingItem("Ensena a la de alta radiacion Moon lo que significa romper un trato");
		api.addBriefingItem("No pierdas la 'Stranger II'; es mi posesion mas valiosa");
		api.addBriefingItem("La nave de Moon tiene mods de motor delicados; usa misiles Salamander para dejarla a la deriva");
		
		boolean testMode = false;
		// Set up the player's fleet.  Variant names come from the
		// files in data/variants and data/variants/fighters
		//api.addToFleet(FleetSide.PLAYER, "station_small_Standard", FleetMemberType.SHIP, "Estacion de pruebas", false);
		if (!testMode) {
			api.addToFleet(FleetSide.PLAYER, "lasher_Standard", FleetMemberType.SHIP, "Stranger II", true);
			api.addToFleet(FleetSide.PLAYER, "hound_d_Standard", FleetMemberType.SHIP, "Viaje de rutina", false);
			
			// Set up the enemy fleet.
			api.addToFleet(FleetSide.ENEMY, "mule_d_pirates_Smuggler", FleetMemberType.SHIP, "Flor de Cherenkov", false);
			
			api.defeatOnShipLoss("Stranger II");
		}
		
		if (testMode) {
//			FleetMemberAPI member = api.addToFleet(FleetSide.PLAYER, "omen_PD", FleetMemberType.SHIP, "Viaje de rutina", true);
...
//			member.getCaptain().getStats().setSkillLevel(Skills.HELMSMANSHIP, 2);
	...
	//		member.setCaptain(person);
			...
			api.addToFleet(FleetSide.ENEMY, "lasher_CS", FleetMemberType.SHIP, "Flor de Cherenkov", false);

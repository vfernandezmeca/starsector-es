...
//		stats.getEngineDamageTakenMult().unmodify(id);
	...
	public StatusData getStatusData(int index, State state, float effectLevel) {
		...
		float damageTakenPercent = EXTRA_DAMAGE_TAKEN_PERCENT * effectLevel;
		if (index == 0) {
			return new StatusData("+" + (int) bonusPercent + "% dano por armas de energia" , false);
		} else if (index == 1) {
			//return new StatusData("+" + (int) damageTakenPercent + "% dano recibido por armas/motores", false);
			return null;
		} else if (index == 2) {
			//return new StatusData("dano recibido por escudo +" + (int) damageTakenPercent + "%", true);
			return null;
		}
		return null;
	}

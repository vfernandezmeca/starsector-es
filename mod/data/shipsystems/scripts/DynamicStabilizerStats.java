...
//		}
	...
	public StatusData getStatusData(int index, State state, float effectLevel) {
		...
		if (index == 0) {
			return new StatusData("maniobrabilidad mejorada", false);
		} else if (index == 1) {
			return new StatusData("cadencia de tiro +" + (int) rof + "%", false);
		} else if (index == 2) {
			return new StatusData("retroceso -" + (int) recoil + "%", false);
		} else if (index == 3) {
			return new StatusData("dano de rayo +" + (int) beam + "%", false);
		}
